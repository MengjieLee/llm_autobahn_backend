...
**示例 1：离线全量模式（无 checkpoint，向后兼容）**

```bash
# 准备含多个 section 的 merged 文件
cat > merged.txt << 'EOF'
__SECTION__:08:47
...
./cache_calc -f merged.txt -s 1000 -b 16 -p true
# 输出:
...
```json
{
  ...
  "k8s_enabled": true,
  "k8s_image": "ccr-xxx.baidubce.com/qianfan-data/llm_autobahn_backend:0.4.0",
